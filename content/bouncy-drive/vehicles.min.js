...
g=new physics.dynamics.Particle(physics.dynamics.Motion.ZERO,e[0].particle.invMass);a.addCollisions([b],[d,e],.5).addVerticalSpringBasic(b,d,(new game.utility.Range(30,60)).multiply(this.SCALING()),new physics.interaction.Elasticity(.06,.15)).addWheelJoint(b,e).addMotor(b,e,2E5,7).addWheelFriction(b,d).addAngleLinkParticles(e[0].particle,g,1,2).addTaillights().addBackgroundBodyImage(resources.vehicles.cargoBike.chassis,c.add((new physics.dynamics.Vector(1122,51)).multiply(this.SCALING())),b).addForegroundGraphics([a.newBodyGraphics(this.newLeg(a,
...
65)).multiply(this.SCALING()),new physics.interaction.Elasticity(.018,.15)).addVerticalSpringBasic(c,h,this.TRAILER_SPRING_RANGE(),new physics.interaction.Elasticity(.018,.15)).addVerticalSpringBasic(c,f,this.TRAILER_SPRING_RANGE(),new physics.interaction.Elasticity(.014,.15)).addAlignDistanceJoint([c,h,f],[b,g,k],m,l,500*this.SCALING(),Math.PI/6,Math.PI/6).addMotor(b,k,8E5,5).addWheelFriction(b,g).addWheelFriction(c,f).addWheelFriction(c,h).addTaillights().addBackgroundBodyImage(resources.vehicles.agriculturalTractor.trailer.chassis,
...
1)}}};vehicles.utility.Setup.MAX_TUNING=10;vehicles.utility.Setup.prototype.tuneExponential=function(a,b,c){return Math.pow(a,1-c*b/vehicles.utility.Setup.MAX_TUNING)};vehicles.utility.Setup.prototype.tuneLinear=function(a,b,c){return a*Math.pow(c,b/vehicles.utility.Setup.MAX_TUNING)};vehicles.utility.Setup.prototype.tuneSpeed=function(a){return this.tuneLinear(a,this.speed,4)};vehicles.utility.Setup.prototype.tuneAcceleration=function(a){return this.tuneLinear(a,this.acceleration,8)};
vehicles.utility.Setup.prototype.tuneTraction=function(a){return this.tuneLinear(a,this.traction,4)};vehicles.utility.Setup.prototype.tuneElasticity=function(a){return new physics.interaction.Elasticity(this.tuneExponential(a.stiffness,this.stiffness,.5),this.tuneExponential(a.damping,this.damping,.5))};vehicles.utility.Setup.prototype.tuneHeight=function(a){return Math.max(a.interpolate(this.height/vehicles.utility.Setup.MAX_TUNING),0)};vehicles.utility.Builder.JOINT_ELASTICITY=function(){return physics.interaction.handlers.elasticity(game.Engine.STIFF_ELASTICITY)};
...
vehicles.utility.Builder.prototype.release=function(){this.bundle.release();this.updaters=[]};vehicles.cargoBike.COST=5E3;vehicles.cargoBike.TUNING_COST_OFFSET=1;vehicles.cargoBike.PROPERTY_PATH=["cargo","bike"];vehicles.cargoBike.SCALING=function(){return game.View.PX_PER_M/934.31};vehicles.cargoBike.BODY_THIGH_ORIGIN=function(){return(new physics.dynamics.Vector(517,-16)).multiply(vehicles.cargoBike.SCALING())};vehicles.cargoBike.CRANK_OFFSET=function(){return(new physics.dynamics.Vector(722,615)).multiply(vehicles.cargoBike.SCALING())};
